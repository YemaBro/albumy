...
from albumy.forms.auth import RegisterForm, LoginForm, ResetPasswordForm
...
def forget_password():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            return redirect(url_for('.login'))
        flash('Invalid email.', 'warnig')
        return redirect(url_for('.forget_password'))
    return render_template('auth/reset_password.html', form=form)


@auth_bp.route('/reset-pasword/<token>', methods=['GET', 'POST'])
def reset_password(token):
    ...
    if form.validate_on_submit():
        ...
        if validate_token(user=user, token=token, operation=Operations.RESET_PASSWORD, new_password=form.password.data):
            ...
            flash('Invalid or expired token.', 'danger')
            return redirect(url_for('.foget_password'))
    return render_template('auth/reset_password.html', form=form)
...
def re_authenticate():
    ...
    form = LoginForm()
    if form.validate_on_submit() and current_user.validete.password(form.password.data):
        confirm_login()
        return render_template('auth/login.html', form=form)
    return render_template('auth/login.html', form=form)
