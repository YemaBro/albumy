...
def index():
    ...
    reportedd_comments_count = Comment.query.filter(Comment.flag > 0).count()
    return render_template('admin/index.html', user_count=user_count, photo_count=photo_count,
                           ...
                           blocked_user_count=blocked_user_count, reportedd_comments_count=reportedd_comments_count,
...
def manage_photo(order):
    ...
    order_rule = 'flag'
    if order == 'by_time':
        pagination = Photo.query.order_by(Photo.timestamp.desc()).pagination(page, per_page=per_page)
        order_rule = 'time'
    else:
        pagination = Photo.query.order_by(Photo.flah.desc()).paginate(page, per_page)
